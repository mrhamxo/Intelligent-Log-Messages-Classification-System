...
model_classification = joblib.load("models/lr_model.joblib")  # Load the pre-trained SVC model


def classify_with_bert(log_message):
    embeddings = model_embedding.encode([log_message])
    probabilities = model_classification.predict_proba(embeddings)[0]  # Get probabilities for each class
